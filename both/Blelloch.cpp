...
vector<Point> findCentersBlelloch(vector<Point> &input, int k){
    int num = input.size();
    ...
    cilk_for(int i = 0; i < num; i++){
        if(maxIndSet[i] == 1){
			centerIndices[indSetScan[i]] = i;
        }
    }
	 
	for(int i = 0; i < k; i++){
		results[i] = input[centerIndices[i]];
	}
...
void swap(vector<Point> &input, int k, vector<Point> &initCenters){
    int num = input.size();
	//create an array for centers
    int* centers = new int[k];
    for(int i = 0; i < k; i++){
		for(int j = 0; j < input.size(); j++){
			if(input[j].distance(initCenters[i]) == 0){
				centers[i] = j;
			}
		}
    }
...
vector<Point> blelloch(vector<Point> input, int k){
    vector<Point> results = findCentersBlelloch(input, k);
	swap(input, k, results);
	return results;
}